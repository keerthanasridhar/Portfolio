 ...
 */
{
  ...
  class Grid {
    constructor(el) {
      ...
      this.filterItemsUI = [];
      Array.from(this.DOM.el.querySelectorAll('.grid__item_ui')).forEach(itemEl => this.filterItemsUI.push(new GridItem(itemEl)));

      this.filterItemsDesign = [];
      Array.from(this.DOM.el.querySelectorAll('.grid__item_design')).forEach(itemEl => this.filterItemsDesign.push(new GridItem(itemEl)));

      this.filterItemsPg = [];
      Array.from(this.DOM.el.querySelectorAll('.grid__item_pg')).forEach(itemEl => this.filterItemsPg.push(new GridItem(itemEl)));
    ...
    initEvents() {
      ...
      // filters
      let filterUiEvent = document.getElementById('filters-project-ui');
      filterUiEvent.addEventListener('click', (ev) => {
        ...
        if (exclude != undefined) {
          ...
          this.sortByDist(refPoint, this.items.filter(item => item != exclude))
			...
			const refPoint = {
			  ...
			  this.sortByDist(refPoint, this.items.filter(item => item))
            .forEach((item, pos) => setTimeout(() => item[show ? 'show' : 'hide'](), show ? 300 + pos * 70 : pos * 70));
		}
      }
    }
